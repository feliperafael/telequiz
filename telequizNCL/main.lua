---TeleQuiz (script lua principal, responsável
...
function main.retangulo()
	  canvas:attrColor("rgba(255,255,255,0.9)")
  	  --Obtém as dimensões da região do script lua (as dimensões do Canvas)
      local w, h = canvas:attrSize()
      h = height or h
...
function main.callback(header, body)
  if body then
     print("\n\n\n", body, "\n\n\n")
  end
end
...
function main.finalizar()
  ...
  elseif percent > 10 and percent < 20 then
  	faixa = 15
  elseif percent > 20 and  percent < 50 then
  	faixa = 35
...
function main.handler(evt) 
  print(evt.class, evt.type, evt.action, evt.name)
  ...
  elseif ((not main.concluido) and evt.class == "ncl" and evt.type == "attribution" and evt.name == "change") then
    main.showInfo(true)
  ...
  elseif evt.class == "ncl" and evt.type == "attribution" and evt.name == "score" then  
  	 
    points = main.finalizar()
    ...
    scoreEvent.value = points
    scoreEvent.action = 'start'; event.post(scoreEvent)
    scoreEvent.action = 'stop';  event.post(scoreEvent)

    	
  elseif (not main.concluido) and string.find ('123456789', evt.key) then
